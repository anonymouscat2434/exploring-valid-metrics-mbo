    ...
    def score_on_dataset(self, 
        ...
        for (eval_postfix, eval_mode) in eval_modes:
            ...
            if sample_fn is None:
                gen_samples = self.sample(tgt_all_y, use_ema=eval_mode)
            else:
            ...
            gen_samples_linspace = self.sample(y_linspace, use_ema=eval_mode)
            y_argmax = cls.predict(gen_samples_linspace).argmax(0)
            samples_y_argmax = self.sample(tgt_all_y[y_argmax].repeat(N_REPEATS,1),
                                           use_ema=eval_mode)
            ...
            if verbose:
                logger.info("mean score computed")
        ...
        logger.info("Time taken for scoring: {:.4f} sec".format(time_taken))
