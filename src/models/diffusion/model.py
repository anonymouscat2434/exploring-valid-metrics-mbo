...
from .networks_unet_mlp import  UNet as EncoderDecoder
...
class Diffusion(BaseModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if gen_kwargs is None:
            gen_kwargs = {}
        self.model = EncoderDecoder(self.n_out, 
                                    n_classes=n_classes, 
                                    **gen_kwargs)
        self.model.to(self.rank)

        if self.use_ema:
            self.model_ema = EncoderDecoder(self.n_out, 
                                            n_classes=n_classes, 
                                            **gen_kwargs)
    ...
    @torch.no_grad()
    def _p_sample(self, x, y, t, t_index, w):
    ...
    def p_sample(self, y, **kwargs):
        ...
        for i in reversed(range(1, self.n_timesteps)):
            img = self._p_sample( 
                img, 
                y,
                torch.full((bs,), i, device=self.rank, dtype=torch.long),
                i,
                w=w
            )
        return img
    ...
    def _run_on_batch(self, batch, train=True, verbose=False, **kwargs):
        ...
        probs = torch.zeros(bs,).uniform_(0, 1).view(-1, 1).to(self.rank)
        if self.tau == 0:
        ...
        if verbose:
            ...
            if self.tau != 0:
                logger.info("dropout rate: {}".format(masks.float().mean()))
