...
class UNet(nn.Module):
    def __init__(
        self,
        channels,           # the number of input dimensions
        dim,                # base multiplier for dim_mults
        n_classes,          # number of diffusion timesteps
        y_max_norm=1.0,
        init_dim=None,
        out_dim=None,
        dim_mults=(1, 2, 4, 8),
        with_time_emb=True,
        with_time_emb_y=True,
        ...
        self.init_conv = nn.Linear(channels, init_dim)
        ...
        self.final_conv = nn.Linear(dim, channels)
    ...
    def forward(self, x, y, time):
        ...
        # bottleneck
        x = self.mid_block1(x, y, t)
        x = self.mid_attn(x)
        x = self.mid_block2(x, y, t)
        ...
        return self.final_conv(self.final_block(x,y))
