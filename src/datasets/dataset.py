...
class GenericDataset(Dataset):
    ...
    def __init__(self, task_name, split="train", test_size=0.5, gain=1.0, gain_y=None):
        super().__init__()
        ...
        logger.debug("({}) X min max = {:.4f} {:.4f}".format(split, X.min(), X.max()))
        logger.debug("({}) y min max = {:.4f} {:.4f}".format(split, y.min(), y.max()))
        ...
        y = self.norm_y(y)
        logger.debug(
            "({}) X min max, shape after normalisation = {:.4f} {:.4f}, {}".format(
                split, X.min(), X.max(), X.shape
            )
        )
        logger.debug(
            "({}) y min max after normalisation = {:.4f} {:.4f}, {}".format(
                split, y.min(), y.max(), y.shape
            )
        )
        ...
        all_X, all_y = self.oracle.internal_dataset.x, self.oracle.internal_dataset.y
        # mask returns all invalid examples, so zero their scores
        mask = self.mask_dataset(all_X, all_y)
        y_masked = np.copy(all_y)
        y_masked[mask] *= 0.
        custom_oracle = self.get_custom_oracle(all_X, y_masked)
        if custom_oracle is not None:
            logger.info("Custom test oracle defined, overriding one in Design Bench with {}".\
                format(custom_oracle))
            if not hasattr(custom_oracle, 'predict'):
                raise Exception("custom_oracle needs predict() method")
